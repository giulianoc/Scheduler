...
Error Scheduler::init(unsigned long ulThreadSleepInMilliSecs, unsigned long ulTimesPointerToAllocOnOverflow)

{

	if (_schedulerStatus != SCHEDULER_BUILDED)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_OPERATION_NOTALLOWED, 1, _schedulerStatus);
...
#endif
	{
		Error err = PThreadErrors(__FILE__, __LINE__, THREADLIB_PTHREAD_INIT_FAILED);
	...
	if (_mtSchedulerMutex.init(PMutex::MUTEX_RECURSIVE) != errNoError)
	{
...
#endif
		{
			Error err = PThreadErrors(__FILE__, __LINE__, THREADLIB_PTHREAD_FINISH_FAILED);
		}
	...
	if ((_pTimesList = new Times_p[ulTimesPointerToAllocOnOverflow]) == (Times_p *)NULL)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_NEW_FAILED);
...

{
	...
	if (bDestroyTimes)
	{
		...
		for (lTimesPointerIndex = 0; lTimesPointerIndex < (long)_ulTimesPointerNumber; lTimesPointerIndex++)
		{
			ptTimes = _pTimesList[lTimesPointerIndex];

			if (ptTimes->finish() != errNoError)
			{
				Error err = SchedulerErrors(__FILE__, __LINE__, SCH_TIMES_FINISH_FAILED);
			}
...

{
	...
	if (_schedulerStatus == SCHEDULER_BUILDED || _schedulerStatus == SCHEDULER_INITIALIZED)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_OPERATION_NOTALLOWED, 1, _schedulerStatus);
...

{
	...
	if (_mtSchedulerMutex.lock() != errNoError)
	{
		_erThreadReturn = PThreadErrors(__FILE__, __LINE__, THREADLIB_PMUTEX_LOCK_FAILED);
	...
	if (_mtSchedulerMutex.unLock() != errNoError)
	{
		_erThreadReturn = PThreadErrors(__FILE__, __LINE__, THREADLIB_PMUTEX_UNLOCK_FAILED);

		return _erThreadReturn;
	}

	ulSecondsToWait = _ulThreadSleepInMilliSecs / 1000;
	ulAdditionalMicroSeconds = (_ulThreadSleepInMilliSecs - (ulSecondsToWait * 1000)) * 1000;

	while (ssSchedulerStatus == SCHEDULER_STARTED || ssSchedulerStatus == SCHEDULER_SUSPENDED)
	{
		if (ssSchedulerStatus == SCHEDULER_STARTED)
		{
			...
			*/
			...
			{
				_erThreadReturn = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULER_HANDLETIMES_FAILED);
...
#ifdef WIN32
		if (WinThread::getSleep(ulSecondsToWait, ulAdditionalMicroSeconds) != errNoError)
#else
		if (PosixThread::getSleep(ulSecondsToWait, ulAdditionalMicroSeconds) != errNoError)
#endif
		{
			_erThreadReturn = PThreadErrors(__FILE__, __LINE__, THREADLIB_PTHREAD_GETSLEEP_FAILED);
...

{
	...
	for (lTimesPointerIndex = 0; lTimesPointerIndex < (long)_ulTimesPointerNumber; lTimesPointerIndex++)
	{
		...
		{
			Error err = SchedulerErrors(__FILE__, __LINE__, SCH_TIMES_ISSTARTED_FAILED);
		...
		if (bIsStarted)
		{
			...
			{
				Error err = SchedulerErrors(__FILE__, __LINE__, SCH_TIMES_ISEXPIREDTIME_FAILED);
			...
			if (bIsExpired)
			{
				...
				if ((errUpdateNextExpirationDateTime = ptTimes->updateNextExpirationDateTime()) != errNoError)
				{
					if ((long)errUpdateNextExpirationDateTime == SCH_REACHED_LASTTIMEOUT)
					{
						bIsTimesDeactived = true;

						if (deactiveTimes(lTimesPointerIndex) != errNoError)
						{
							Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULER_DEACTIVETIMES_FAILED);
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_TIMES_UPDATENEXTEXPIRATIONDATETIME_FAILED);
				...
				*/
				...
				{
					Error err = SchedulerErrors(__FILE__, __LINE__, SCH_TIMES_HANDLETIMEOUT_FAILED);
...

{
	...
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULER_TIMESALREADYACTIVE);
	...
	if (_ulTimesPointerNumber == _ulAllocatedTimesPointerNumber)
	{
		...
		if ((_pTimesList = new Times_p[_ulAllocatedTimesPointerNumber]) == (Times_p *)NULL)
		{
			Error err = SchedulerErrors(__FILE__, __LINE__, SCH_NEW_FAILED);
...

{
	...
	if (getTimesPointerIndex(pTimes, &lTimesPointerIndex) != errNoError)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULER_GETTIMESPOINTERINDEX_FAILED);
...

{
	...
	if (lTimesPointerIndex < 0 || lTimesPointerIndex >= (long)_ulTimesPointerNumber)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_ACTIVATION_WRONG);
	...
	for (lTimesPointerIndex++; lTimesPointerIndex < (long)_ulTimesPointerNumber; lTimesPointerIndex++)
	{
...
Error Scheduler::getTimes(unsigned long ulTimesIndex, Times_p *pTimes)

{
	...
	if (ulTimesIndex >= _ulTimesPointerNumber || pTimes == (Times_p *)NULL)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_ACTIVATION_WRONG);
...
Error Scheduler::getSchedulerStatus(SchedulerStatus_p pssSchedulerStatus)
...
Error Scheduler::getTimesPointerIndex(Times_p pTimes, long *plTimesPointerIndex)

{
	...
	if (pTimes == (Times_p)NULL || plTimesPointerIndex == (long *)NULL)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_ACTIVATION_WRONG);
	...
	for (*plTimesPointerIndex = 0; *plTimesPointerIndex < (long)_ulTimesPointerNumber; (*plTimesPointerIndex)++)
	{
	...
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_TIMES_NOTFOUND);
