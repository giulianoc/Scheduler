...
Times::Times(void) { _schTimesStatus = SCHTIMES_BUILDED; }
...
Error Times::init(unsigned long ulPeriodInMilliSeconds, const char *pClassName)

{

	if (_schTimesStatus != SCHTIMES_BUILDED)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_OPERATION_NOTALLOWED, 1, _schTimesStatus);
	...
	{
		Error err = PThreadErrors(__FILE__, __LINE__, THREADLIB_PMUTEX_INIT_FAILED);
	...
	if (pClassName != (const char *)NULL)
	{
		...
		{
			Error err = SchedulerErrors(__FILE__, __LINE__, SCH_CLASSNAME_TOOLONG);
...

{
	...
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_ACTIVATION_WRONG);
	...
	// checks on pSchedule parameter
	{
		...
		// check spaces number and characters (digit, '*', ',', '-', ' ')
		{
			...
			for (lScheduleIndex = 0; lScheduleIndex < lScheduleLength; lScheduleIndex++)
			{
		...
		// check range values
		{
			...
			// year
			if ((pScheduleToken = strtok_r(pScheduleCopy, " ", &pScheduleTokenLast)) == (char *)NULL)
			{
				Error err = SchedulerErrors(__FILE__, __LINE__, SCH_CALENDARSCHEDULE_WRONG);
			...
			if ((pScheduleToken = strtok_r((char *)NULL, " ", &pScheduleTokenLast)) == (char *)NULL)
			{
				Error err = SchedulerErrors(__FILE__, __LINE__, SCH_CALENDARSCHEDULE_WRONG);
			...
			if (strchr(pScheduleToken, ',') != (char *)NULL)
			{
				...
				do
				{
					if (strchr(pPointerToScheduleToken, ',') == (char *)NULL)
						lNumberLength = strlen(pPointerToScheduleToken);
					else
						lNumberLength = strchr(pPointerToScheduleToken, ',') - pPointerToScheduleToken;

					strncpy(pNumber, pPointerToScheduleToken, lNumberLength);

					pNumber[lNumberLength] = '\0';

					if (strchr(pPointerToScheduleToken, ',') == (char *)NULL)
						pPointerToScheduleToken += lNumberLength;
					else
						pPointerToScheduleToken += (lNumberLength + 1);

					lValue = atol(pNumber);

					if (lValue < 1 || lValue > 12)
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValue, "month");
			...
			else if (strchr(pScheduleToken, '-') != (char *)NULL)
			{
				...
				lNumberLength = strchr(pScheduleToken, '-') - pScheduleToken;
				...
				*/
				...
				{
					Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValueFrom, "month");
				...
				if (lValueTo > 12)
				{
					Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValueTo, "month");
			...
			{ // single value or '*'
				...
				if (pScheduleToken[0] != '*')
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lMonth, "month");
			...
			if (strchr(pScheduleToken, ',') != (char *)NULL)
			{
				...
				do
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValue, "monthday");
				...
				*/
				...
				{
					Error err = SchedulerErrors(
						__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValue, "monthday (with '*' as year and february as month)"
					);
			...
			else if (strchr(pScheduleToken, '-') != (char *)NULL)
			{
				...
				*/
				...
				// if year is *, month is february and day is 30 or 31
				// we have a loop
				if (!strcmp(pYear, "*") && lMonth == 2 && lValueFrom == 30 && lValueTo == 31)
				{
					Error err = SchedulerErrors(
						__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValueFrom, "monthday (with '*' as year and february as month)"
					);
			...
			{ // single value or '*'
				...
				if (pScheduleToken[0] != '*')
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lMonthDay, "monthday");
					...
					{
						Error err = SchedulerErrors(
							__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lMonthDay, "monthday (with '*' as year and february as month)"
						);
			...
			if (strchr(pScheduleToken, ',') != (char *)NULL)
			{
				...
				do
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValue, "weekday");
			...
			{ // single value or '*'
				...
				if (pScheduleToken[0] != '*')
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lWeekDay, "weekday");
			...
			if (strchr(pScheduleToken, ',') != (char *)NULL)
			{
				...
				do
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValue, "hour");
			...
			{ // single value or '*'
				...
				if (pScheduleToken[0] != '*')
				{
					...
					if (lHour < 0 || lHour > 23)
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lHour, "hour");
			...
			if (strchr(pScheduleToken, ',') != (char *)NULL)
			{
				...
				do
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValue, "minute");
			...
			{ // single value or '*'
				...
				if (pScheduleToken[0] != '*')
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lMinute, "minute");
			...
			if (strchr(pScheduleToken, ',') != (char *)NULL)
			{
				...
				do
				{
					...
					if (lValue < 0 || lValue > 59)
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lValue, "second");
			...
			{ // single value or '*'
				...
				if (pScheduleToken[0] != '*')
				{
					...
					{
						Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SCHEDULEFIELDRANGE_WRONG, 2, lSecond, "second");
...

{
	...
	if (_mtTimesMutex.finish() != errNoError)
	{
		Error err = PThreadErrors(__FILE__, __LINE__, THREADLIB_PMUTEX_FINISH_FAILED);
...

{
	...
	{
		Error err = PThreadErrors(__FILE__, __LINE__, THREADLIB_PMUTEX_LOCK_FAILED);
	...
	if (_schTimesStatus != SCHTIMES_INITIALIZED)
	{
		...
		{
			Error err = PThreadErrors(__FILE__, __LINE__, THREADLIB_PMUTEX_UNLOCK_FAILED);
	...
	if (pStartDateTime != (const char *)NULL)
	{
		...
		if (sscanf(
				pStartDateTime, "%4lu-%2lu-%2lu %2lu:%2lu:%2lu:%4lu", &ulSrcYear, &ulSrcMonth, &ulSrcDay, &ulSrcHour, &ulSrcMinutes, &ulSrcSeconds,
				&ulSrcMilliSeconds
			) != 7)
		{
			Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SSCANF_FAILED);
		...
		Datetime::addSeconds(
			ulSrcYear, ulSrcMonth, ulSrcDay, ulSrcHour, ulSrcMinutes, ulSrcSeconds, -1, 0, &ulDestYear, &ulDestMonth, &ulDestDay, &ulDestHour,
			&ulDestMinutes, &ulDestSeconds, &_bCurrentDaylightSavingTime
		);

		sprintf(
			_pCurrentExpirationDateTime, "%04lu-%02lu-%02lu %02lu:%02lu:%02lu:%04lu", ulDestYear, ulDestMonth, ulDestDay, ulDestHour, ulDestMinutes,
			ulDestSeconds, ulDestMilliSeconds
		);
	}
	else
	{
		...
		sprintf(
			_pCurrentExpirationDateTime, "%04lu-%02lu-%02lu %02lu:%02lu:%02lu:%04lu", (unsigned long)(tmDateTime.tm_year + 1900),
			(unsigned long)(tmDateTime.tm_mon + 1), (unsigned long)(tmDateTime.tm_mday), (unsigned long)(tmDateTime.tm_hour),
			(unsigned long)(tmDateTime.tm_min), (unsigned long)(tmDateTime.tm_sec), ulMilliSecs
		);
	}
	...
	if (_ttTimesType == SCH_TYPE_PERIODIC)
	{
		...
		if ((errUpdateNextPeriodicExpirationDateTime = updateNextPeriodicExpirationDateTime()) != errNoError)
		{
			if ((long)errUpdateNextPeriodicExpirationDateTime != SCH_REACHED_LASTTIMEOUT)
			{
				Error err = SchedulerErrors(__FILE__, __LINE__, SCH_TIMES_UPDATENEXTPERIODICEXPIRATIONDATETIME_FAILED);
	...
	{ // SCH_TYPE_CALENDAR
		...
		if ((errUpdateNextCalendarExpirationDateTime = updateNextCalendarExpirationDateTime()) != errNoError)
		{
			// Error err = SchedulerErrors (__FILE__, __LINE__,
...

{
	...
	// if (tmDateTime. tm_isdst == 1 && !_bNextDaylightSavingTime)	// this check is wrong because in case of milliseconds period, tmDateTime.
	// tm_isdst == 1 could not mean 'moving from DST to No DST' but could mean 'moving from No DST and we just entered in DST from few millisecs'. So
	// the correct check uses instead _bCurrentDaylightSavingTime
	if (tmDateTime.tm_isdst == 1 && _bCurrentDaylightSavingTime && !_bNextDaylightSavingTime)
	{
	...
	else if (tmDateTime.tm_isdst == 0 && _bCurrentDaylightSavingTime && _bNextDaylightSavingTime)
	{
		// 2011-11-02:
		//  Scenario: 02:59:00 (DST) ... 03:00:00=02:00:00 (No DST) ...
	...
	else
	{
		// the format date is yyyy-mm-dd-hh24-mi-ss-milliss
		sprintf(
			pDateTime, "%04lu-%02lu-%02lu %02lu:%02lu:%02lu:%04lu", (unsigned long)(tmDateTime.tm_year + 1900),
			(unsigned long)(tmDateTime.tm_mon + 1), (unsigned long)(tmDateTime.tm_mday), (unsigned long)(tmDateTime.tm_hour),
			(unsigned long)(tmDateTime.tm_min), (unsigned long)(tmDateTime.tm_sec), ulMilliSecs
		);
...

{
	...
	else // SCH_TYPE_CALENDAR
	{
		...
		if ((errUpdateNextCalendarExpirationDateTime = updateNextCalendarExpirationDateTime()) != errNoError)
		{
			if ((long)errUpdateNextCalendarExpirationDateTime == SCH_REACHED_LASTTIMEOUT)
				_schTimesStatus = SCHTIMES_INITIALIZED;
...

{
	...
	if (addMilliSecondsToDateTime(
			_pNextExpirationDateTime, &_bNextDaylightSavingTime, _pCurrentExpirationDateTime, _bCurrentDaylightSavingTime, _ulPeriodInMilliSeconds
		) != errNoError)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_TIMES_ADDSECONDSTODATETIME_FAILED);
...
Error Times::addMilliSecondsToDateTime(
	char *pNextDateTime, Boolean_p pbNextDaylightSavingTime, const char *pCurrentDateTime, long lCurrentDaylightSavingTime,
...

{
	...
	if (pNextDateTime == (char *)NULL || pCurrentDateTime == (char *)NULL)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_ACTIVATION_WRONG);
	...
	if (sscanf(
			pCurrentDateTime, "%4lu-%2lu-%2lu %2lu:%2lu:%2lu:%4lu", &ulCurrentYear, &ulCurrentMonth, &ulCurrentDay, &ulCurrentHour, &ulCurrentMinutes,
			&ulCurrentSeconds, &ulCurrentMilliSeconds
		) != 7)
	{
		Error err = SchedulerErrors(__FILE__, __LINE__, SCH_SSCANF_FAILED);
	...
	ulNextMilliSeconds = (ulCurrentMilliSeconds + ulMilliSeconds) - (ulSecondsToAdd * 1000);

	Datetime::addSeconds(
		ulCurrentYear, ulCurrentMonth, ulCurrentDay, ulCurrentHour, ulCurrentMinutes, ulCurrentSeconds, lCurrentDaylightSavingTime,
		(long long)ulSecondsToAdd, &ulNextYear, &ulNextMonth, &ulNextDay, &ulNextHour, &ulNextMinutes, &ulNextSeconds, pbNextDaylightSavingTime
	);

	sprintf(
		pNextDateTime, "%04lu-%02lu-%02lu %02lu:%02lu:%02lu:%04lu", ulNextYear, ulNextMonth, ulNextDay, ulNextHour, ulNextMinutes, ulNextSeconds,
		ulNextMilliSeconds
	);
...

{
	...
	// initialize the tm struct (the tm_wday field too)
	{
		...
		if (sscanf(
				_pCurrentExpirationDateTime, "%4lu-%2lu-%2lu %2lu:%2lu:%2lu:%4lu", &ulLocalYear, &ulLocalMonth, &ulLocalDay, &ulLocalHour,
...
#else
		if (localtime_r(&lUtcTime, &tmNextExpirationDateTime) == (struct tm *)NULL)
#endif
		{
			Error err = SchedulerErrors(__FILE__, __LINE__, SCH_LOCALTIME_R_FAILED);
	...
	while (!bIsDateTimeValid)
	{
		...
		// year
		if ((pScheduleToken = strtok_r(pSchedule, " ", &pScheduleTokenLast)) == (char *)NULL)
		{
			Error err = SchedulerErrors(__FILE__, __LINE__, SCH_CALENDARSCHEDULE_WRONG);
		...
		if (strchr(pScheduleToken, ',') != (char *)NULL)
		{
			...
			*/
			...
			if (!bIsYearValid)
			{
				...
				{
					Error err = SchedulerErrors(__FILE__, __LINE__, SCH_REACHED_LASTTIMEOUT);
		...
		else if (strchr(pScheduleToken, '-') != (char *)NULL)
		{
			...
			*/
			...
			if (!(lValueFrom <= tmNextExpirationDateTime.tm_year + 1900 && tmNextExpirationDateTime.tm_year + 1900 <= lValueTo))
			{
		...
		else if (strchr(pScheduleToken, '-') != (char *)NULL)
		{
			...
			*/
			...
			if (!(lValueFrom <= tmNextExpirationDateTime.tm_mon + 1 && tmNextExpirationDateTime.tm_mon + 1 <= lValueTo))
			{
				//	initialize tmNextExpirationDateTime to the first minute
				//	of the next month
				{
...

		{
			...
			else if (strchr(pScheduleToken, '-') != (char *)NULL)
			{
				...
				*/
				...
				if (!(lValueFrom <= tmNextExpirationDateTime.tm_mday && tmNextExpirationDateTime.tm_mday <= lValueTo))
				{
					bIsNecessaryAddDayForMonthDay = true;
				}
			}
			...
			else if (strchr(pScheduleToken, '-') != (char *)NULL)
			{
				...
				*/
				...
				if (!(lValueFrom <= tmNextExpirationDateTime.tm_wday && tmNextExpirationDateTime.tm_wday <= lValueTo))
				{
					bIsNecessaryAddDayForWeekDay = true;
				}
			}
			...
			{ // !bIsMonthDayStar && !bIsWeekDayStar
				if (bIsNecessaryAddDayForMonthDay && bIsNecessaryAddDayForWeekDay)
					bIsNecessaryAddDay = true;
				else
					bIsNecessaryAddDay = false;
			}
		...
		else if (strchr(pScheduleToken, '-') != (char *)NULL)
		{
			...
			*/
			...
			if (!(lValueFrom <= tmNextExpirationDateTime.tm_hour && tmNextExpirationDateTime.tm_hour <= lValueTo))
			{
				// add 1 hour
				{
					time_t lUtcTime;
		...
		else if (strchr(pScheduleToken, '-') != (char *)NULL)
		{
			...
			*/
			...
			if (!(lValueFrom <= tmNextExpirationDateTime.tm_min && tmNextExpirationDateTime.tm_min <= lValueTo))
			{
				// add 1 minute
				{
					time_t lUtcTime;
		...
		else if (strchr(pScheduleToken, '-') != (char *)NULL)
		{
			...
			*/
			...
			if (!(lValueFrom <= tmNextExpirationDateTime.tm_sec && tmNextExpirationDateTime.tm_sec <= lValueTo))
			{
				// add 1 second
				{
					time_t lUtcTime;
	...
	sprintf(
		_pNextExpirationDateTime, "%04lu-%02lu-%02lu %02lu:%02lu:%02lu:%04lu", (unsigned long)(tmNextExpirationDateTime.tm_year + 1900),
		(unsigned long)(tmNextExpirationDateTime.tm_mon + 1), (unsigned long)(tmNextExpirationDateTime.tm_mday),
		(unsigned long)(tmNextExpirationDateTime.tm_hour), (unsigned long)(tmNextExpirationDateTime.tm_min),
		(unsigned long)(tmNextExpirationDateTime.tm_sec), ulMilliSeconds
	);
